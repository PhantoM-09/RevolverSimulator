    ...
    void Update()
    {
        if (!_isMove)
        {
            if (TriggerTarget.OpenForDog)
            {
                ...
                gameObject.transform.rotation = Quaternion.Euler(new Vector3(value, gameObject.transform.rotation.eulerAngles.y, gameObject.transform.rotation.eulerAngles.z));
        ...
        {
            if (HigherTriggerTarget.OpenForDogBack)
            {
                if (_offset < 1)
                    _offset += HelperTarget.Speed;
