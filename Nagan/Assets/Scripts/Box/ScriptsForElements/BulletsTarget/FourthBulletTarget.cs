...
public class FourthBulletTarget : MonoBehaviour
{
    ...
    void Update()
    {
        if (!RetryButton.FourthBulletBack)
        {
            if (HigherTriggerTarget.OpenForFourthBullet)
            {
                if (_offset < 1)
        ...
        else
        {
            ...
            gameObject.transform.localPosition = newPosition;
            RetryButton.FourthBulletBack = false;
        }
    }
}
